    ...
    const fetchPreviousPapersMetadata = async () => {
      try {
        const response = await axios.get(
          `http://localhost:5000/api/admin/previouspapers-metadata/${subject}`
        );
  ...
  const handleUpload = async () => {
    ...
    try {
      const response = await axios.post(
        "http://localhost:5000/api/admin/upload-previouspapers",
        formData,
        {
  ...
  const handleDelete = async (filename) => {
    try {
      const response = await axios.delete(
        `http://localhost:5000/api/admin/previouspapers/filename/${filename}`
      );
          ...
          {uploadedFiles.length > 0 &&
            uploadedFiles.map((fileData) => (
              <div key={fileData.fileId}>
                <IframePreview
                  src={`http://localhost:5000/api/admin/previouspapers-file/${fileData.fileId}`}
